...
export function validateDeployment(deployment) {
    ...
    for (const container of deployment.containers) {
        ...
        for (const port of container.ports) {
            ...
            if (portErrors)
                validationErrors = validationErrors.concat(portErrors)
        }
        ...
        if (container.volumeMounts) {
            for (const volumeMount of container.volumeMounts) {
                ...
                if (volumeMountErrors)
                    validationErrors =
                        validationErrors.concat(volumeMountErrors)
            }
        }
    }
    ...
    if (deployment.volumes) {
        ...
        if (volumesErrors)
            validationErrors = validationErrors.concat(volumesErrors)
        else {
                ...
                const volumeErrors = validate(volume, {
                    ...
                    size: {
                        ...
                        format: {
                            ...
                            message:
                                'must be one to three digits followed by Mi or Gi',
                        },
                    },
                })
                if (volumeErrors)
